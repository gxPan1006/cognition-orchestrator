defmodule Cognition.Config.Schema do
  @moduledoc false
  ...
  defmodule Workspace do
    ...
    @primary_key false
    embedded_schema do
      field(:root, :string, default: Path.join(System.tmp_dir!(), "cognition_workspaces"))
    end
  ...
  defmodule Claude do
    ...
    @primary_key false
    embedded_schema do
      field(:command, :string, default: "claude --print --verbose --output-format stream-json --permission-mode bypassPermissions")
      field(:turn_timeout_ms, :integer, default: 3_600_000)
      field(:stall_timeout_ms, :integer, default: 300_000)
    end
  ...
  embedded_schema do
    ...
    embeds_one(:coding_tool, CodingTool, on_replace: :update, defaults_to_struct: true)
  ...
  defp changeset(attrs) do
    ...
    |> cast_embed(:coding_tool, with: &CodingTool.changeset/2)
  ...
  defp finalize_settings(settings) do
    ...
    workspace = %{
      settings.workspace
      | root: resolve_path_value(settings.workspace.root, Path.join(System.tmp_dir!(), "cognition_workspaces"))
    }
    ...
    coding_tool = %{
      settings.coding_tool
      | kind: normalize_tool_kind(settings.coding_tool.kind)
    }

    %{settings | tracker: tracker, workspace: workspace, coding_tool: coding_tool, codex: codex}
  end

  defp normalize_tool_kind(kind) when is_binary(kind) do
    kind
    |> String.trim()
    |> String.downcase()
  end

  defp normalize_tool_kind(kind), do: kind
  ...
  defp expand_local_workspace_root(_workspace_root) do
    Path.expand(Path.join(System.tmp_dir!(), "cognition_workspaces"))
  end
