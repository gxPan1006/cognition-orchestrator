defmodule Cognition.SSH do
  ...
  def run(host, command, opts \\ []) when is_binary(host) and is_binary(command) do
    ssh_bin = Keyword.get(opts, :ssh_bin)
    cmd_opts = Keyword.delete(opts, :ssh_bin)

    with {:ok, executable} <- ssh_executable(ssh_bin) do
      {:ok, System.cmd(executable, ssh_args(host, command), cmd_opts)}
    end
  end
  ...
  def start_port(host, command, opts \\ []) when is_binary(host) and is_binary(command) do
    ssh_bin = Keyword.get(opts, :ssh_bin)
    opts = Keyword.delete(opts, :ssh_bin)
  ...
  defp ssh_executable(ssh_bin) when is_binary(ssh_bin) and ssh_bin != "" do
    {:ok, ssh_bin}
  end

  defp ssh_executable(_ssh_bin) do
    case configured_ssh_executable() do
  ...
  defp configured_ssh_executable do
    System.get_env("COGNITION_SSH_BIN") || System.get_env("SYMPHONY_SSH_BIN")
  end
