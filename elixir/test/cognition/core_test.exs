defmodule Cognition.CoreTest do
  ...
  test "config defaults and validation checks" do
    ...
    assert config.coding_tool.kind == "codex"
    ...
    write_workflow_file!(Workflow.workflow_file_path(), coding_tool_kind: "claude")
    assert Config.settings!().coding_tool.kind == "claude"
    assert Config.coding_tool_kind() == "claude"
    assert :ok = Config.validate!()

    write_workflow_file!(Workflow.workflow_file_path(), coding_tool_kind: "bad-tool")
    assert {:error, {:unsupported_coding_tool, "bad-tool"}} = Config.validate!()
  end
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    coding_tool = Map.get(config, "coding_tool", %{})
    assert Map.get(coding_tool, "kind") == "codex"
  ...
  test "Cognition.start_link delegates to the orchestrator" do
    write_workflow_file!(Workflow.workflow_file_path(), tracker_kind: "memory")
    Application.put_env(:cognition, :memory_tracker_issues, [])
    orchestrator_pid = Process.whereis(Cognition.Orchestrator)

    on_exit(fn ->
      if is_nil(Process.whereis(Cognition.Orchestrator)) do
        case Supervisor.restart_child(Cognition.Supervisor, Cognition.Orchestrator) do
          {:ok, _pid} -> :ok
    ...
    if is_pid(orchestrator_pid) do
      assert :ok = Supervisor.terminate_child(Cognition.Supervisor, Cognition.Orchestrator)
    end

    assert {:ok, pid} = Cognition.start_link()
    assert Process.whereis(Cognition.Orchestrator) == pid
  ...
  test "missing running issues stop active agents without cleaning the workspace" do
    ...
    previous_memory_issues = Application.get_env(:cognition, :memory_tracker_issues)
    ...
    try do
      ...
      assert {:ok, workspace} =
               Cognition.PathSafety.canonicalize(Path.join(test_root, issue_identifier))
  ...
  defp restore_app_env(key, nil), do: Application.delete_env(:cognition, key)
  defp restore_app_env(key, value), do: Application.put_env(:cognition, key, value)
  ...
  test "prompt builder preserves unicode workflow prompt and issue fields" do
    workflow_prompt = """
    Language:
    - 默认使用简体中文输出所有面向人的内容，包括 Linear workpad 评论、清单项、状态说明。
    ...
    issue = %Issue{
      identifier: "GXP-10",
      title: "修复账号系统和订阅问题",
      description: "修复 data root、sub2api 和 device-link 返回目标。",
      state: "Todo",
      url: "https://example.org/issues/GXP-10",
      labels: ["backend"]
    }
    ...
    assert prompt =~ "默认使用简体中文"
    assert prompt =~ "修复账号系统和订阅问题"
    assert Jason.encode!(%{"text" => prompt})
  end
  ...
  test "prompt builder reports workflow load failures separately from template parse errors" do
    original_workflow_path = Workflow.workflow_file_path()
    workflow_store_pid = Process.whereis(Cognition.WorkflowStore)

    on_exit(fn ->
      Workflow.set_workflow_file_path(original_workflow_path)

      if is_pid(workflow_store_pid) and is_nil(Process.whereis(Cognition.WorkflowStore)) do
        Supervisor.restart_child(Cognition.Supervisor, Cognition.WorkflowStore)
      end
    end)

    assert :ok = Supervisor.terminate_child(Cognition.Supervisor, Cognition.WorkflowStore)
