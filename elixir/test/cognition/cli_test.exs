defmodule Cognition.CLITest do
  ...
  test "returns the guardrails acknowledgement banner when the flag is missing" do
    ...
    assert banner =~ "This Cognition implementation is a low key engineering preview."
    assert banner =~ "The configured coding tool will run without any guardrails."
    assert banner =~ "Cognition is not a supported product and is presented as-is."
  ...
  test "returns startup error when app cannot start" do
    ...
    assert {:error, message} = CLI.evaluate([@ack_flag, "WORKFLOW.md"], deps)
    assert message =~ "Failed to start Cognition with workflow"
    assert message =~ ":boom"
  end
