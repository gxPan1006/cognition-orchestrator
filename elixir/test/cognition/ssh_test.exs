defmodule Cognition.SSHTest do
  ...
  test "run/3 returns an error when ssh is unavailable" do
    ...
    previous_path = System.get_env("PATH")
    previous_ssh_bin = System.get_env("COGNITION_SSH_BIN")

    on_exit(fn ->
      restore_env("PATH", previous_path)
      restore_env("COGNITION_SSH_BIN", previous_ssh_bin)
      File.rm_rf(test_root)
    end)

    File.mkdir_p!(test_root)
    System.put_env("PATH", test_root)
    System.delete_env("COGNITION_SSH_BIN")
  ...
  test "start_port/3 supports binary output without line mode" do
    ...
    assert {:ok, port} = SSH.start_port("localhost", "printf ok", ssh_bin: fake_ssh)
    assert is_port(port)
  ...
  test "start_port/3 supports line mode" do
    ...
    assert {:ok, port} = SSH.start_port("localhost:2222", "printf ok", line: 256, ssh_bin: fake_ssh)
    assert is_port(port)
    assert_receive {^port, {:data, {:eol, "ready"}}}, 3_000
    wait_for_trace!(trace_file)
  ...
  defp install_fake_ssh!(test_root, trace_file, script \\ nil) do
    ...
    System.put_env("COGNITION_SSH_BIN", fake_ssh)
