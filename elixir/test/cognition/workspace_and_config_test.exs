defmodule Cognition.WorkspaceAndConfigTest do
  use Cognition.TestSupport
  alias Cognition.Config.Schema
  alias Cognition.Config.Schema.{Codex, StringOrMap}
  alias Cognition.Linear.Client
  ...
  test "workspace replaces stale non-directory paths" do
    ...
    try do
      ...
      assert {:ok, canonical_workspace} = Cognition.PathSafety.canonicalize(stale_workspace)
  ...
  test "workspace rejects symlink escapes under the configured root" do
    ...
    try do
      ...
      assert {:ok, canonical_outside_root} = Cognition.PathSafety.canonicalize(outside_root)
      assert {:ok, canonical_workspace_root} = Cognition.PathSafety.canonicalize(workspace_root)
  ...
  test "workspace canonicalizes symlinked workspace roots before creating issue directories" do
    ...
    try do
      ...
      assert {:ok, canonical_workspace} =
               Cognition.PathSafety.canonicalize(Path.join(actual_root, "MT-LINK"))
  ...
  test "workspace remove rejects the workspace root itself with a distinct error" do
    ...
    try do
      ...
      assert {:ok, canonical_workspace_root} =
               Cognition.PathSafety.canonicalize(workspace_root)
  ...
  test "linear client paginates issue state fetches by id beyond one page" do
    ...
    assert query =~ "CognitionLinearIssuesById"
  ...
  test "workspace remove continues when before_remove hook times out" do
    previous_timeout = Application.get_env(:cognition, :workspace_hook_timeout_ms)

    on_exit(fn ->
      if is_nil(previous_timeout) do
        Application.delete_env(:cognition, :workspace_hook_timeout_ms)
      else
        Application.put_env(:cognition, :workspace_hook_timeout_ms, previous_timeout)
      end
    end)

    Application.put_env(:cognition, :workspace_hook_timeout_ms, 10)
  ...
  test "config reads defaults for optional settings" do
    ...
    assert config.workspace.root == Path.join(System.tmp_dir!(), "cognition_workspaces")
    ...
    assert {:ok, canonical_default_workspace_root} =
             Cognition.PathSafety.canonicalize(Path.join(System.tmp_dir!(), "cognition_workspaces"))
  ...
  test "schema parse normalizes policy keys and env-backed fallbacks" do
    ...
    assert settings.workspace.root == Path.join(System.tmp_dir!(), "cognition_workspaces")
  ...
  test "schema resolves sandbox policies from explicit and default workspaces" do
    ...
    assert Schema.resolve_turn_sandbox_policy(%Schema{
             ...
             "type" => "workspaceWrite",
             "writableRoots" => [Path.expand(Path.join(System.tmp_dir!(), "cognition_workspaces"))],
             "readOnlyAccess" => %{"type" => "fullAccess"},
             "networkAccess" => false,
             "excludeTmpdirEnvVar" => false,
             "excludeSlashTmp" => false
  ...
  test "path safety returns errors for invalid path segments" do
    ...
    assert {:error, {:path_canonicalize_failed, ^expanded_path, :enametoolong}} =
             Cognition.PathSafety.canonicalize(path)
  end
