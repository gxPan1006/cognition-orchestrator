...
5. For `Todo` tickets, do startup sequencing in this exact order:
   ...
   - find/create `## Cognition Workpad` bootstrap comment
...
5.  Ensure the workpad includes a compact environment stamp at the top as a code fence line:
    ...
    - Example: `devbox-01:/home/dev-user/code/cognition-workspaces/MT-32@7bdde33bc`
...
8.  Attach PR URL to the issue (prefer attachment; use the workpad comment only if attachment is unavailable).
    - Ensure the GitHub PR has label `cognition` (add it if missing).
...
4. Remove the existing `## Cognition Workpad` comment from the issue.
5. Create a fresh branch from `origin/main`.
6. Start over from the normal kickoff flow:
   - If current issue state is `Todo`, move it to `In Progress`; otherwise keep the current state.
   - Create a new bootstrap `## Cognition Workpad` comment.
...
- Required PR metadata is present (`cognition` label).
...
- Use exactly one persistent workpad comment (`## Cognition Workpad`) per issue.
